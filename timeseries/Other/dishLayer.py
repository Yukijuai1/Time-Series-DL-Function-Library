...
class DishTS(nn.Module):
    def __init__(self, n_series, seq_len, init):
    ...
    def loss_prior(self, y, weight):
        y = torch.unsqueeze(y, dim=-1)
        y = torch.unsqueeze(y, dim=-1)
        return weight * (torch.pow(torch.mean((y - self.phih)), 2))
    
def dishLayer(n_series,seq_len,init='standard'):
    return DishTS(n_series, seq_len, init)